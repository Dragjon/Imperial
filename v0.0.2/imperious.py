...
def qsearch(ply, alpha, beta):
    ...
    legals = sorted(captures, key=lambda move: 
    ...
    for move in captures:
        delta_index = piece_to_index[board.piece_at(move.from_square).piece_type]
        if delta_index < 5 and stand_pat + deltas[delta_index] < alpha:
            return alpha
...
def play_chess():
    ...
    while True:
        line = input()
        if line == "uci":
            print("id name Imperious v0.0.2")
