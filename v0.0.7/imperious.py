...
def alphabeta(depth, ply, alpha, beta, parent_move):
    ...
    for move in legals:
        ...
        if not pv_node and not board.is_capture(move) and depth <= fp_depth and seval * fp_margin < alpha and abs(max) < mate_score - ply - 1:
            continue
        ...
        if move_count == 1 and pv_node:
            ...
            if lmr > 0 and score > alpha:
                score = -alphabeta(depth + check_extension - 1, ply + 1, -alpha - 1, -alpha, move)
...
def play_chess():
    global board, hbtm, sbtm, hb_max_time, sb_max_time, start_time, tt_size, tt_move, hash_size_mb, null_move_r, rfp_depth, rfp_margin, lmr_count, lmr_depth, lmr_base, lmr_mul, deltas, asp_delta, asp_depth
    remaining_time = 1000000

    while True:
        line = input()
        if line == "uci":
            print("id name Imperious v0.0.7")
            print("id author Dragjon")
            print(f"option name Hash type spin default 32 min 1 max 256") 
            print(f"option name NullMoveR type spin default 3 min 0 max 10") # 1
            print(f"option name RFPMargin type spin default 55 min 0 max 100") # 5
            print(f"option name RFPDepth type spin default 8 min 0 max 20") # 2
            print(f"option name LMRCount type spin default 5 min 0 max 15") # 1
            print(f"option name LMRDepth type spin default 2 min 0 max 10") # 1
            print(f"option name LMRBaseInt type spin default 750 min 0 max 5000") # 100
            print(f"option name LMRMulInt type spin default 400 min 0 max 5000") # 100
            print(f"option name DeltaPawn type spin default 180 min 0 max 480") # 10
            print(f"option name DeltaKnight type spin default 400 min 0 max 800") # 10
            print(f"option name DeltaBishop type spin default 450 min 0 max 850") # 10
            print(f"option name DeltaRook type spin default 550 min 0 max 950") # 10
            print(f"option name DeltaQueen type spin default 1100 min 0 max 1500") # 10
            print(f"option name ASPDepth type spin default 3 min 0 max 10") # 1
            print(f"option name ASPDelta type spin default 40 min 0 max 200") # 5
            print(f"option name HBTM type spin default 10 min 1 max 30") # 5
            print(f"option name SBTM type spin default 40 min 1 max 60") # 5
            print("uciok")
