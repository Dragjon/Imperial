...
def alphabeta(board, depth, ply, alpha, beta, parent_move):
    global nodes, seldepth, best_move
...
def play_chess():
    global board, hbtm, sbtm, hb_max_time, sb_max_time, start_time, tt_size, tt_move, hash_size_mb, null_move_r, rfp_depth, rfp_margin, lmr_count, lmr_depth, lmr_base, lmr_mul, deltas, asp_delta, asp_depth, fp_depth, fp_margin, tempo
    remaining_time = 1000000

    while True:
        line = input()
        if line == "uci":
            print("id name Imperious v0.0.8")
            print("id author Dragjon")
            print(f"option name Hash type spin default {hash_size_mb} min 1 max 256") 
