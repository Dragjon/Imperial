...
def qsearch(ply, alpha, beta):
    ...
    legals = sorted(captures, key=lambda move: 10_000_000 if move == move_in_tt else 0, reverse=True)
...
def alphabeta(depth, ply, alpha, beta):
    ...
    board_hash = chess.polyglot.zobrist_hash(board) % tt_size
    move_in_tt = tt_move[board_hash]
    legals = sorted(list(legals), key=lambda move: 10_000_000 if move == move_in_tt else 0, reverse=True)
    for move in legals:
        nodes += 1
        board.push(move)
...
def play_chess():
    ...
    while True:
        line = input()
        if line == "uci":
            print("id name Imperious v0.0.1")
